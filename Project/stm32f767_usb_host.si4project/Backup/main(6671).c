...
static void LOG_Task(void const *argument)
{
	...
	while(1)
	{
		if(recv_buf_p->p_write - recv_buf_p->p_read > 0)
		{
			__PRINT_LOG__(__CRITICAL_LEVEL__, "Recv count %d: %s\r\n", \
				recv_buf_p->p_write - recv_buf_p->p_read, \
				recv_buf_p->buffer_base + (recv_buf_p->p_read & mask));
			recv_buf_p->p_read = recv_buf_p->p_write;
		}
		delay_ms(10);
	}
}
...
void USBH_CDC_ReceiveCallback(USBH_HandleTypeDef *phost)
{
	...
	CDC_Handle->pRxData[USBH_CDC_GetLastReceivedDataSize(phost)] = '\0';
	if((log_buf.p_write & mask) + USBH_CDC_GetLastReceivedDataSize(phost) + 1 > mask)
	{
		unsigned int len = LOG_BUFF_SIZE - (log_buf.p_write & mask);
		memcpy(dest_buff, CDC_Handle->pRxData, len);
		memcpy(log_buf.buffer_base, CDC_Handle->pRxData + len, USBH_CDC_GetLastReceivedDataSize(phost) + 1 - len);
	}
	else
	{
		memcpy(dest_buff, CDC_Handle->pRxData, USBH_CDC_GetLastReceivedDataSize(phost));
	}
	  ...
	  case APPLICATION_RUNNING:
		...
		Appli_state = APPLICATION_RUNNING;
  		...
  		{  			
	        ...
	        while(1)
	        {
              snprintf(send_buf, 64, "%d: %s", count++, "hello world!\r\n");
	          USBH_CDC_Transmit(&hUSBHost, (unsigned char *)send_buf, strlen(send_buf));			  
			  __PRINT_LOG__(__CRITICAL_LEVEL__, "Send: %s", send_buf);
			  /*if(0 == recv_flag)
			  {
